---
title: "Permissions (雙語/Bilingual)"
sidebarTitle: "Permissions (雙語)"
...
<LanguageSelector current="bilingual" path="/developers/bilingual/topics/permissions" />
...
<Info>
[Application command permissions](/developers/en-us/interactions/application-commands#permissions) allow you to enable or disable specific commands for entire channels in addition to individual roles or users.
</Info>
...
<Info>
[應用程式指令權限 (Application command permissions)](/developers/bilingual/interactions/application-commands#permissions) 允許您除了設定特定身分組或使用者之外，還能為整個頻道阻擋或允許特定指令。
</Info>
...
<Info>
In [API v6 (now deprecated)](/developers/en-us/reference), the `permissions`, `allow`, and `deny` fields in roles and overwrites are still serialized as a number; however, these numbers shall not grow beyond 31 bits. During the remaining lifetime of API v6, all new permission bits will only be introduced in `permissions_new`, `allow_new`, and `deny_new`. These `_new` fields are just for response serialization; requests with these fields should continue to use the original `permissions`, `allow`, and `deny` fields, which accept both string or number values.
</Info>
...
<Info>
在 [API v6（現已棄用）](/developers/bilingual/reference) 中，身分組和覆寫權限中的 `permissions`、`allow` 和 `deny` 欄位仍然序列化為數字；然而，這些數字不得超過 31 位元。在 API v6 的剩餘生命週期中，所有新的權限位元都將僅在 `permissions_new`、`allow_new` 和 `deny_new` 中引入。這些 `_new` 欄位僅用於回應序列化；使用這些欄位的請求應繼續使用接受字串或數字值的原始 `permissions`、`allow` 和 `deny` 欄位。
</Info>
...
**\* These permissions require the owner account to use [two-factor authentication](/developers/en-us/topics/oauth2#twofactor-authentication-requirement) when used on a guild that has server-wide 2FA enabled.**

**\*\* See [Permissions for Timed Out Members](/developers/en-us/topics/permissions#permissions-for-timed-out-members) to understand how permissions are temporarily modified for timed out users.**
...
**\* 當在啟用了全伺服器 2FA 的伺服器上使用時，這些權限要求擁有者帳戶啟用 [雙重身分驗證 (2FA)](/developers/bilingual/topics/oauth2#twofactor-authentication-requirement)。**

**\*\* 請參閱 [被停權成員的權限](/developers/bilingual/topics/permissions#permissions-for-timed-out-members) 以了解如何暫時修改被停權使用者的權限。**
...
| colors         | [role colors](/developers/en-us/topics/permissions#role-object-role-colors-object) object | the role's colors                                                                                                                     |
| hoist          | boolean                                                                             | if this role is pinned in the user listing                                                                                            |
| icon?          | ?string                                                                             | role [icon hash](/developers/en-us/reference#image-formatting)                                                                              |
...
| tags?          | [role tags](/developers/en-us/topics/permissions#role-object-role-tags-structure) object  | the tags this role has                                                                                                                |
| flags          | integer                                                                             | [role flags](/developers/en-us/topics/permissions#role-object-role-flags) combined as a [bitfield](https://en.wikipedia.org/wiki/Bit_field) |
...
| colors         | [身分組顏色](/developers/bilingual/topics/permissions#role-object-role-colors-object) 物件 | 身分組的顏色設定 |
| hoist          | boolean                                                                             | 此身分組是否釘選在使用者列表中顯示 |
| icon?          | ?string                                                                             | 身分組 [圖示雜湊值 (icon hash)](/developers/bilingual/reference#image-formatting) |
...
| tags?          | [身分組標籤](/developers/bilingual/topics/permissions#role-object-role-tags-structure) 物件  | 此身分組擁有的標籤 |
| flags          | integer                                                                             | 身分組 [旗標](/developers/bilingual/topics/permissions#role-object-role-flags)，組合成 [位元欄位](https://en.wikipedia.org/wiki/Bit_field) |
...
This object will always be filled with `primary_color` being the role's `color`. Other fields can only be set to a non-null value if the guild has the `ENHANCED_ROLE_COLORS` [guild feature](/developers/en-us/resources/guild#guild-object-guild-features).
...
此物件一律會填入身分組的 `color` 作為 `primary_color`。只有在伺服器具有 `ENHANCED_ROLE_COLORS` [伺服器特徵 (guild feature)](/developers/bilingual/resources/guild#guild-object-guild-features) 時，其他欄位才能設定為非 null 值。
...
|-----------|----------|----------------------------------------------------------------------------------------------------------------|
| IN_PROMPT | `1 << 0` | role can be selected by members in an [onboarding](/developers/en-us/resources/guild#guild-onboarding-object) prompt |
...
| 旗標 | 數值 | 說明 |
|-----------|----------|----------------------------------------------------------------------------------------------------------------|
| IN_PROMPT | `1 << 0` | 身分組可在[新手引導過程 (onboarding)](/developers/bilingual/resources/guild#guild-onboarding-object)中供成員選擇 |
