...
The rest of the guide assumes you've already developed an [app](/developers/quick-start/overview-of-apps) that can launch an Activity. If you aren't at that point quite yet, you should follow the guide on [building your first Activity](/developers/activities/building-an-activity) before continuing.</div><div>v>
...
本指南的後續部分均假設您已經開發了一個可以啟動互動活動的 [應用程式](/developers/zh-tw/quick-start/overview-of-apps)。如果您尚未進行到此步驟，您應該在繼續之前，遵循 [建置您的第一個互動活動](/developers/zh-tw/activities/building-an-activity) 指南進行。</div></div>v>
...
3. The state `(1 of max_party)` badge will only render when a party field is provided. Otherwise, state will be shown in a line of text below details.</div><div>v>
    <div style={{ borderBottom: "2px solid #5865f2", paddingBottom: "8px", marginBottom: "16px", fontWeight: "bold", fontSize: "18px" }}>🇹🇼 繁體中文</div>
...
3. `(1 of max_party)` 狀態勳章只會在提供了 party (隊伍) 欄位時渲染。否則，該狀態將會以一行文字的形式顯示在詳細資訊下方。</div></div>v>
