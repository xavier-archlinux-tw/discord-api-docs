  ...
  const verifyLayout = async (pageName: string) => {
    ...
    if (biLayout) {
      ...
      if (biLayout.containerY < biLayout.proseBottomY - 10) {
        console.error(
          `❌ 視覺佈局驗證失敗：導航元件位置 (${biLayout.containerY}) 沒有在正文最下方 (${biLayout.proseBottomY})！`,
        );
        process.exit(1);
      }
    } else {
    ...
    if (menuState) {
      console.log(
        `${pageName}語言選單底部診斷：Y軸=${menuState.y}，bottom屬性=${menuState.bottomStyle}，position=${menuState.positionStyle}，zIndex=${menuState.zIndex}，可見=${menuState.isInViewport}，頂層元素=${menuState.topTagName}`,
      );
      ...
      if (menuState.bottomStyle !== "112px") {
        console.error(
          `❌ 驗證失敗：${pageName}語言列在最底部時沒有避讓（bottom 應為 112px，實際為 ${menuState.bottomStyle}）`,
        );
        process.exit(1);
      }
