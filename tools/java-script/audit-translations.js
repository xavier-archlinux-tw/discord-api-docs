...
function runAudit() {
  ...
  TARGET_PAIRS.forEach(({ en: enDir, zh: zhDir, bi: biDir }) => {
    ...
    enFiles.forEach(enPath => {
      ...
      const displayPath = path.join(path.basename(enDir) === 'en-us' ? path.basename(path.dirname(enDir)) : '', relPath).replace(/\\/g, '/');
      const fileReport = {
        relPath: displayPath,
        zhExists: !!zhData,
        biExists: !!biData,
      ...
      if (zhData) {
        if (zhData.headings.length < enData.headings.length) {
          fileReport.issues.push(`zh-tw has fewer headings than en-us (${zhData.headings.length} vs ${enData.headings.length})`);
        }
        if (zhData.codeBlocksCount < enData.codeBlocksCount) {
          fileReport.issues.push(`zh-tw has fewer code blocks than en-us (${zhData.codeBlocksCount} vs ${enData.codeBlocksCount})`);
        }
        ...
        if (zhData.infoCount < enData.infoCount) {
          fileReport.issues.push(`zh-tw has fewer <Info> boxes than en-us (${zhData.infoCount} vs ${enData.infoCount})`);
        }
        if (zhData.warningCount < enData.warningCount) {
          fileReport.issues.push(`zh-tw has fewer <Warning> boxes than en-us (${zhData.warningCount} vs ${enData.warningCount})`);
        }
      } else {
      ...
      if (biData) {
        if (biData.headings.length < enData.headings.length) {
          fileReport.issues.push(`bilingual has fewer headings than en-us (${biData.headings.length} vs ${enData.headings.length})`);
        }
        if (biData.codeBlocksCount < enData.codeBlocksCount) {
          fileReport.issues.push(`bilingual has fewer code blocks than en-us (${biData.codeBlocksCount} vs ${enData.codeBlocksCount})`);
        }
        if (biData.imagesCount < enData.imagesCount) {
          fileReport.issues.push(`bilingual has fewer images than en-us (${biData.imagesCount} vs ${enData.imagesCount})`);
        }
      } else {
  ...
  console.log(`Files with actual layout/content discrepancies (among existing translations): ${existingDiscrepancies.length}`);
